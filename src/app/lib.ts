...
// fixed follow tslint tips by xuqinghan @ 20171009
...
class Hex {
    ...
    [new Hex(2, -1, -1), new Hex(1, -2, 1), new Hex(-1, -1, 2), new Hex(-2, 1, 1), new Hex(-1, 2, -1), new Hex(1, 1, -2)];
    ...
    public static round(h: Hex): Hex {
        let q: number = Math.round(h.q);
        let r: number = Math.round(h.r);
        let s: number = Math.round(h.s);
        const q_diff: number = Math.abs(q - h.q);
        const r_diff: number = Math.abs(r - h.r);
        const s_diff: number = Math.abs(s - h.s);
    ...
    public static linedraw(a: Hex, b: Hex): Hex[] {
        const N: number = Hex.distance(a, b);
        const a_nudge: Hex = new Hex(a.q + 0.000001, a.r + 0.000001, a.s - 0.000002);
...
class Layout {
    ...
    new Orientation(Math.sqrt(3.0), Math.sqrt(3.0) / 2.0, 0.0, 3.0 / 2.0, Math.sqrt(3.0) / 3.0, -1.0 / 3.0, 0.0, 2.0 / 3.0, 0.5);
    public static flat: Orientation =
    new Orientation(3.0 / 2.0, 0.0, Math.sqrt(3.0) / 2.0, Math.sqrt(3.0), 2.0 / 3.0, 0.0, -1.0 / 3.0, Math.sqrt(3.0) / 3.0, 0.0);

    public static hexToPixel(layout: Layout, h: Hex): Point {
        const M: Orientation = layout.orientation;
        const size: Point = layout.size;
        const origin: Point = layout.origin;
    ...
    public static hexCornerOffset(layout: Layout, corner: number): Point {
        const M: Orientation = layout.orientation;
        const size: Point = layout.size;
        const angle: number = 2.0 * Math.PI * (M.start_angle - corner) / 6;
    ...
    public static polygonCorners(layout: Layout, h: Hex): Point[] {
        const corners: Point[] = [];
        const center: Point = Layout.hexToPixel(layout, h);
        for (let i = 0; i < 6; i++) {
            const offset: Point = Layout.hexCornerOffset(layout, i);
...
class Tests {
    ...
    public static testHexRound(): void {
        const a: Hex = new Hex(0, 0, 0);
        const b: Hex = new Hex(1, -1, 0);
        const c: Hex = new Hex(0, -1, 1);
        ...
        Tests.equalHex('hex_round 4', Hex.round(a), Hex.round(
            new Hex(a.q * 0.4 + b.q * 0.3 + c.q * 0.3, a.r * 0.4 + b.r * 0.3 + c.r * 0.3, a.s * 0.4 + b.s * 0.3 + c.s * 0.3)));
        Tests.equalHex('hex_round 5', Hex.round(c), Hex.round(
            new Hex(a.q * 0.3 + b.q * 0.3 + c.q * 0.4, a.r * 0.3 + b.r * 0.3 + c.r * 0.4, a.s * 0.3 + b.s * 0.3 + c.s * 0.4)));
    }
    ...
    public static testHexLinedraw(): void {
        ...
        [new Hex(0, 0, 0), new Hex(0, -1, 1), new Hex(0, -2, 2), new Hex(1, -3, 2), new Hex(1, -4, 3), new Hex(1, -5, 4)],
         Hex.linedraw(new Hex(0, 0, 0), new Hex(1, -5, 4)));
    }


    public static testLayout(): void {
        const h: Hex = new Hex(3, 4, -7);
        const flat: Layout = new Layout(Layout.flat, new Point(10, 15), new Point(35, 71));
        Tests.equalHex('layout', h, Hex.round(Layout.pixelToHex(flat, Layout.hexToPixel(flat, h))));
        const pointy: Layout = new Layout(Layout.pointy, new Point(10, 15), new Point(35, 71));
    ...
    public static testConversionRoundtrip(): void {
        ...
        Tests.equalHex('conversion_roundtrip even-q', 
        a, OffsetCoord.qoffsetToCube(OffsetCoord.EVEN, OffsetCoord.qoffsetFromCube(OffsetCoord.EVEN, a)));
        Tests.equalOffsetcoord('conversion_roundtrip even-q',
         b, OffsetCoord.qoffsetFromCube(OffsetCoord.EVEN, OffsetCoord.qoffsetToCube(OffsetCoord.EVEN, b)));
        Tests.equalHex('conversion_roundtrip odd-q',
         a, OffsetCoord.qoffsetToCube(OffsetCoord.ODD, OffsetCoord.qoffsetFromCube(OffsetCoord.ODD, a)));
        Tests.equalOffsetcoord('conversion_roundtrip odd-q',
         b, OffsetCoord.qoffsetFromCube(OffsetCoord.ODD, OffsetCoord.qoffsetToCube(OffsetCoord.ODD, b)));
        Tests.equalHex('conversion_roundtrip even-r',
         a, OffsetCoord.roffsetToCube(OffsetCoord.EVEN, OffsetCoord.roffsetFromCube(OffsetCoord.EVEN, a)));
        Tests.equalOffsetcoord('conversion_roundtrip even-r',
         b, OffsetCoord.roffsetFromCube(OffsetCoord.EVEN, OffsetCoord.roffsetToCube(OffsetCoord.EVEN, b)));
        Tests.equalHex('conversion_roundtrip odd-r',
         a, OffsetCoord.roffsetToCube(OffsetCoord.ODD, OffsetCoord.roffsetFromCube(OffsetCoord.ODD, a)));
        Tests.equalOffsetcoord('conversion_roundtrip odd-r',
         b, OffsetCoord.roffsetFromCube(OffsetCoord.ODD, OffsetCoord.roffsetToCube(OffsetCoord.ODD, b)));
    }


    public static testOffsetFromCube(): void {
        Tests.equalOffsetcoord('offset_from_cube even-q',
         new OffsetCoord(1, 3), OffsetCoord.qoffsetFromCube(OffsetCoord.EVEN, new Hex(1, 2, -3)));
        Tests.equalOffsetcoord('offset_from_cube odd-q',
         new OffsetCoord(1, 2), OffsetCoord.qoffsetFromCube(OffsetCoord.ODD, new Hex(1, 2, -3)));
    }
